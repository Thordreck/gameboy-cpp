...
TEST_CASE("8 bit add overflows are properly detected")
{
...
TEST_CASE("8 bit half add overflows are properly detected")
{
	CHECK_FALSE(utils::check_half_add_overflow(0x0_u8, 0x0_u8));
	CHECK(utils::check_half_add_overflow(0x0F_u8, 0x01_u8));
}

TEST_CASE("8 bit substract underflows are properly detected")
{
...
TEST_CASE("8 bit half substract underflows are properly detected")
{
	CHECK_FALSE(utils::check_substract_underflow(0xF_u8, 0xF_u8));
	CHECK_FALSE(utils::check_substract_underflow(0xF_u8, 0xA_u8));
	CHECK(utils::check_substract_underflow(0x0A_u8, 0x0B_u8));
...
TEST_CASE("16 bit add overflows are properly detected")
{
...
TEST_CASE("16 bit substract underflows are properly detected")
{
...
TEST_CASE("16 bit half substract underflows are properly detected")
{
