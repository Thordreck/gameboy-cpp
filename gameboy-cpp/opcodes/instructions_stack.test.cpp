...
namespace
{
    ...
    tests::r16_read_memory_test_case<opcodes::pop_bc, tests::test_memory, tests::get_bc>, \
    tests::r16_read_memory_test_case<opcodes::pop_de, tests::test_memory, tests::get_de>, \
    tests::r16_read_memory_test_case<opcodes::pop_hl, tests::test_memory, tests::get_hl>

	#define push_r16_test_cases \
    tests::r16_write_memory_test_case<opcodes::push_bc, tests::test_memory, tests::get_bc>, \
    tests::r16_write_memory_test_case<opcodes::push_de, tests::test_memory, tests::get_de>, \
    tests::r16_write_memory_test_case<opcodes::push_hl, tests::test_memory, tests::get_hl>
}
...
TEST_CASE("stack.ld_sp_n16 updates sp with correct value from memory")
{
	...
	tests::execute_all_instruction_steps<opcodes::ld_sp_n16>(cpu, memory);
...
TEST_CASE("stack.add_sp_e8 updates stack pointer properly")
{
	...
	tests::execute_all_instruction_steps<opcodes::add_sp_e8>(cpu, memory);
	CHECK_EQ(cpu.sp, 127);
...
TEST_CASE("stack.ld_hl_sp_e8 updates hl properly")
{
	...
	tests::execute_all_instruction_steps<opcodes::ld_hl_sp_e8>(cpu, memory);
...
TEST_CASE("stack.pop_af updates reg with correct value from stack")
{
	...
	tests::execute_all_instruction_steps<opcodes::pop_af>(cpu, memory);
...
TEST_CASE("stack.push_af updates stack with correct value from register")
{
	...
	tests::execute_all_instruction_steps<opcodes::push_af>(cpu, memory);

	CHECK_EQ(memory.read(stack_origin - 2), 0xC0);
	CHECK_EQ(memory.read(stack_origin - 1), 0xAB);
	CHECK_EQ(cpu.pc, 0);
	CHECK_EQ(cpu.sp, stack_origin - 2);
}
