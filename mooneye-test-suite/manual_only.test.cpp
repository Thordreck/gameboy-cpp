...
TEST_CASE("mooneye.manual_only.sprite_priority")
{
    ...
    auto cartridge = require_success(cartridge::load_rom_file("roms/manual-only/roms/sprite_priority.gb"));
    ...
    const std::filesystem::path output_filepath = std::filesystem::temp_directory_path() / "sprite_priority_test.png";
    ...
    using namespace stb;
    const stb_result<image> reference_img = load_image("roms/manual-only/reference/sprite_priority-expected.png", graphics::num_color_channels);
